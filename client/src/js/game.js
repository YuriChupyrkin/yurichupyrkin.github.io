class Game {
  ...
  init() {
    ...
    // subsrcibe
    this._socketHelper.onPlayerNotified((serverGameState) => {
      this._refreshCirclesResolver.updateServerState(serverGameState);
    });
  }
  ...
  refresh() {
    ...
    const resolvedCircles = this._refreshCirclesResolver.resolve();
    if (!resolvedCircles) {
      return;
    }
